...
function wzStartPlacementCountdown(seconds) {
  const box = document.getElementById('wz-timer-box');
...
function renderPendingGames(container, games) {
  ...
  container.innerHTML = games.map(g => `
    <div style="display:flex;align-items:center;gap:8px;padding:10px;border:1px solid #ddd;border-radius:8px;margin-bottom:8px;">
      ...
      ${g.creator_id === currentUser.id || g.opponent_id === currentUser.id ? `<button class="btn-small" onclick="cancelPendingGame('${g.id}')">Delete</button>` : ''}
    </div>
  `).join('');
}
...
function makeChoice(choice) {
  ...
  // Disable buttons, highlight the chosen one
...
socket.on('round_started', (data) => {
  ...
  myChoice = null;
  document.querySelectorAll('.choice-btn').forEach(btn => { btn.disabled = false; btn.classList.remove('picked'); });
...
async function loadWithdrawalRequests() {
  ...
  try {
    ...
    container.innerHTML = '<h3 style="font-size:16px;color:#333;margin-bottom:10px">My Withdrawal Requests</h3>';
    if (reqs.length === 0) {
...
async function loadAdminRequests(status = '') {
  ...
  try {
    ...
    noteEl.textContent = reqs.length ? `${reqs.length} request(s) shown` : 'No requests found';
    if (reqs.length === 0) {
...
async function rejectWithdrawalRequest(id) {
  ...
  showModal('Reject and refund?', 'The held balance will be returned to the player.', async () => {
    try {
      ...
      showToast('Request rejected, balance refunded', 'success');
      loadAdminRequests('pending');
    } catch (err) { showToast(err.message, 'error'); }
  }, () => {});
}
